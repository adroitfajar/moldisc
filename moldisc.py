...
def moldisc(project_name = "data_1",
    ...
    while True:
        ...
        if iter == 0: 
            ...
            my_env["CUDA_VISIBLE_DEVICES"] = sub_GPU_ids #"0,1,2,3"  # Subprocess to run the GPT2
            subprocess.run(['conda', 'run', '-n', 'subGPT', 'python', 'mainGPT.py', '--iter' , str(iter) , '--args_project_folder' ,  project_folder , '--args_project_name' ,   project_name], env = my_env)
            ...
            my_env = os.environ.copy()
            my_env["CUDA_VISIBLE_DEVICES"] = sub_GPU_ids
            subprocess.run(['conda', 'run', '-n', 'subGPT', 'python', 'mainGPT.py', '--iter' , str(iter) , '--args_project_folder' ,  project_folder , '--args_project_name' ,   project_name], env=my_env)
